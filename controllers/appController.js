...
module.exports.initializeTransaction = async (req, res) => {
  ...
  const proofOfDepositFile = req.file || null;
  ...
  if(transactionStatus, transactionType, amount, currency, TransactorWalletAddress, TransactorEmailAddress, TransactorUsername){
    try{
      if(proofOfDepositFile && transactionType === 'deposit'){
        const newTransaction = await Transactions.create({transactionType, transactionStatus, amount, currency, TransactorWalletAddress, TransactorEmailAddress, TransactorUsername, proofOfDeposit: proofOfDepositFile.filename});
      ...
      WalletDB.findOne({$and: [{ownerUsername: username}, {ownerEmail: email}]}).then(result => {
        if(result){
          ...
          const walletBalance = [{
            ...
            accountBalance: (20.00).toString() + " USD",
            coins: [
                {
                    name: 'BTC',
                ...
                {
                    name: 'USDC',
                    balance: (parseFloat((result.usdc).toString()).toFixed(2))
                }
            ]
        }]
